...
const WorksheetCard = ({ isLoading }: WorksheetCardProps) => {
  ...
  return (
    <>
      {isLoading ? (
        <div className="flex-1 flex place-content-center place-items-center uppercase text-gray-900 font-[family-name:var(--font-geist-sans)]">
          Loading...
        </div>
      ) : (
        <div className="flex-1 bg-gray-100 text-gray-900 rounded-md flex flex-col gap-2 h-full">
          <div className="flex-1 flex gap-2">
            <PanelGroup direction="horizontal">
              ...
              <PanelResizeHandle className="bg-gray-300 w-[1px] h-full" />
              <Panel>
                <PanelGroup direction="vertical">
                  <Panel defaultSize={25} className="flex flex-col gap-2 p-2">
                    <WorksheetInput fetchQueryOutput={fetchQueryOutput} />
                  </Panel>
                  <PanelResizeHandle className="bg-gray-300 w-full h-[1px]" />
                  <Panel>
                    <WorksheetOutput />
                  </Panel>
                </PanelGroup>
